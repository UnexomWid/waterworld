...
COLOR_LINE_1 = pygame.Color(0xffcc33ff)
...
class TextField(Widget):
    def __init__(self, w: int, text: Optional[str] = None, color_font=COLOR_FONT_NORMAL,
                 color_background=COLOR_BACKGROUND_1, align='left', font_name=FONT_NAME, font_size=FONT_SIZE, padding=1):
        super().__init__()
        assert w > 1
        assert align in ('left', 'right', 'middle')
        assert font_size > 5
    ...
    def set_content(self, text: str):
        ...
        if self.align == 'middle':
            self._surface.blit(
                text_surface, (self._surface.get_width() / 2 - text_surface.get_width() / 2 - 2, self.padding))
        if self.align == 'right':
            self._surface.blit(
    ...
    def update(self):
        self.set_content(self.text)


class LinePlot(Widget):
    def __init__(self, w: int, h: int, color_background=COLOR_BACKGROUND_1, y_ints=False):
        super().__init__()
        assert w > 1 and h > 1
        self.w, self.h = w, h
        self.color_background = color_background
    ...
    def set_content(self, x, y, y_range=None):
        ...
        scale_x = self.w / max(0.0001, (x_max - x_min))
        x = (x - x_min)*scale_x
        ...
        if not self.y_ints:
            ...
            text_y_min = self.font.render(f'{int(self.y_min)}', True, COLOR_FONT_NORMAL, self.color_background)
            text_y_max = self.font.render(f'{int(self.y_max)}', True, COLOR_FONT_NORMAL, self.color_background)
